...
export default function OrgInsights({ results }: { results: DiagnosticResult[] }) {
  ...
  const generatePDF = (org: OrgData, showParticipants: boolean, fullyAnonymized: boolean = false, teamReportFor?: string | null) => {
    ...
    // (Seniority breakdown shown in executive dashboard above)
    ...
    if (weakDimensions.length > 0) {
      ...
      weakDimensions.forEach(([key, score], index) => {
        ...
        const blockH = 10 + 6 + 5 + (weekLines.length * lineH) + 8 + 5 + (monthLines.length * lineH) + 8 + cascadeH + changeActivitiesH + metricsH + 8;
