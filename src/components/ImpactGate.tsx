...
const PASSWORD = "Liz4Impact 2026";
const STORAGE_KEY = "impact-deck-access";

export function ImpactGate({ children }: { children: React.ReactNode }) {
  const [unlocked, setUnlocked] = useState(false);
  const [value, setValue] = useState("");
  const [error, setError] = useState(false);

  useEffect(() => {
    // Inject noindex meta for bots/LLMs
    ...
    <div className="flex min-h-screen items-center justify-center bg-background p-6">
      <form onSubmit={submit} className="w-full max-w-sm space-y-4 rounded-lg border border-border bg-card p-6 shadow-sm">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">This deck is private. Enter the access password to continue.</p>
        </div>
        <Input
          type="password"
          autoFocus
