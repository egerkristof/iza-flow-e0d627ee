...
/**
 * /satcom-brief — Responsive one-page summary of the Satcom Operator Strategic Deck.
 *
 * Council reframe (vs. /space-brief which targets BUILDERS):
 * Satellite OPERATORS (Hispasat, SES, Eutelsat, Inmarsat, Telesat) don't build
 * the satellite — they procure it, fly it for 15 years, and answer to regulators
 * and customers the whole way. Their lifecycle knowledge sits in:
 *   - Fleet operations (anomaly playbooks, station-keeping, EOL)
 *   - Procurement governance (managing primes, acceptance, contracts)
 *   - Regulatory & spectrum (ITU filings, orbital coordination)
 *   - Service continuity (SLAs, gov/defense, sovereignty)
 *
 * Open-canvas hook: the category has no defined standard yet. The first
 * operator to codify it becomes the reference architecture for the rest.
 *
 * Single CTA: View the full deck (/satcom).
 */

const TEAL = "200 95% 38%";        // satcom-cyan, distinguishes from space deck teal
const MINT = "180 90% 42%";

const verticalProblems = [
  {
    stat: "15–20 yrs",
    label: "Fleet memory at risk",
    detail:
      "Operations engineers who know each satellite's quirks — its drift, its anomalies, its workarounds — retire with the bird. The next ops generation starts the learning curve from zero.",
    source: "Satcom operator field engagements",
  },
  {
    stat: "6–18 mo",
    label: "Procurement learning loop",
    detail:
      "Replacement satellites are procured every 7–10 years. The contract managers, acceptance leads, and spectrum specialists who learned the last cycle are rarely the ones running the next.",
    source: "Satcom procurement benchmarks",
  },
  {
    stat: "$M / hour",
    label: "Outage exposure",
    detail:
      "Broadcast, government, and IFC SLAs convert anomaly response time directly into revenue and contract penalties. The playbook for each rare event lives in one or two senior heads.",
    source: "Industry SLA frameworks",
  },
];
...
const loop = [
  {
    step: "Codify",
    short: "Capture fleet & procurement memory.",
    desc: "Turn anomaly playbooks, SOPs, vendor SOWs, and ITU precedent into governed, reusable rules.",
    Icon: Satellite,
  },
  {
    step: "Govern",
    short: "Check every AI output.",
    desc: "AI answers are gated against your operations procedures, regulatory commitments, and SLA framework.",
    Icon: Shield,
  },
  {
    step: "Apply",
    short: "Operators ramp and run faster.",
    desc: "New ops engineers, contract managers, and analysts work inside the same governed standard — not around it.",
    Icon: Radio,
  },
  {
    step: "Improve",
    short: "Lessons compound across the fleet.",
    desc: "Every anomaly, every procurement cycle, every regulatory filing feeds new learning back. The next satellite starts smarter.",
    Icon: RefreshCw,
  },
];

export default function SatcomBrief() {
  return (
    <div className="min-h-screen bg-white text-foreground">
      ...
      <main className="mx-auto max-w-5xl px-5 sm:px-8 pt-10 sm:pt-16 pb-24">
        ...
        >
          For Satellite Operators · 2-Minute Brief
        </motion.p>
        ...
        >
          Satellites last 15 years.<br className="hidden sm:block" />
          <span> </span>The people who know how to fly them{" "}
          <span style={{ color: `hsl(${TEAL})` }}>don't.</span>
        </motion.h1>
        ...
        >
          Fleet operations, procurement governance, and spectrum knowledge live in senior heads. AI doesn't have it either.
        </motion.p>

        {/* The Operator Context Gap, as one iceberg */}
        <motion.div
        ...
        >
          <BriefIceberg
            ...
            belowLabel="What actually keeps the fleet flying"
            above={["SOPs", "ITU filings", "Vendor SOWs", "SLA contracts"]}
            below={[
              "Per-satellite drift, anomalies & workarounds",
              "Procurement & acceptance precedent",
              "Spectrum coordination memory",
              "Sovereign / gov / IFC SLA judgment",
            ]}
          />
        </motion.div>
        ...
        <motion.p
          ...
          style={{ color: `hsl(${TEAL})` }}
        >
          LIZA OS is the Operator Memory Layer — codifies fleet operations and procurement intelligence, ramps the next ops generation faster, and governs every AI output against your standards.
        </motion.p>

        {/* Open canvas callout — distinct from /space-brief */}
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="mt-10 sm:mt-14 rounded-2xl border-2 p-6 sm:p-8"
        ...
        >
          <p
            ...
            style={{ color: `hsl(${TEAL})` }}
          >
            Open Canvas · Define the Standard
          </p>
          <p className="font-black text-xl sm:text-3xl tracking-tight leading-tight">
            No operator has codified this layer yet. The first one to do it becomes the reference architecture for the category.
          </p>
          <p className="mt-3 text-sm sm:text-base text-muted-foreground leading-snug max-w-3xl">
            ECSS exists for builders. ITIL exists for IT. There is no governed knowledge standard for satellite operations and procurement. We are building it with the first operator who steps in — and they own a strategic position in it.
          </p>
        </motion.div>
        ...
        <section className="mt-14 sm:mt-20">
          ...
          <div className="relative">
            ...
            <div className="hidden sm:flex items-center justify-center mt-8 gap-2 text-[11px] font-bold tracking-widest uppercase text-muted-foreground">
              ...
              <span>Each cycle makes the next satellite, contract, and filing smarter</span>
            </div>
          </div>
        </section>
        ...
        >
          ...
          <p className="font-black text-2xl sm:text-4xl tracking-tight">
            One workflow. One satellite. 30 days.
          </p>
          <p className="mt-3 text-sm sm:text-lg text-muted-foreground leading-snug max-w-2xl">
            We pick one operations workflow — anomaly response, station-keeping handover, or vendor acceptance — codify it against your standards, and measure the hours saved and risk reduced. You get a clear before-and-after, and an audit trail.
          </p>
        </section>
        ...
        <footer className="mt-16 pt-6 border-t border-border flex items-center justify-between text-[11px] text-muted-foreground">
          <span className="font-bold text-foreground">LIZA OS · Operator memory, governed.</span>
          <span>lizaos.ai/satcom-brief</span>
        </footer>
      </main>
    </div>
  );
